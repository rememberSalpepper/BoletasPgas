...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return NextResponse.json({ error: `Error del backend: ${text}` }, { status: res.status });
    }
    ...
    if (msg.toLowerCase().includes("fetch failed") || msg.toLowerCase().includes("econnrefused")) {
      return NextResponse.json({ error: `Error interno al conectar con la API backend: ${msg}` }, { status: 502 });
    }
    return NextResponse.json({ error: `Error interno en API Route: ${msg}` }, { status: 500 });
  }
}
